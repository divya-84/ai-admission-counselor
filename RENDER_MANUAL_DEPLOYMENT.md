...
| `FRONTEND_URL` | **Enter Manually** | The production Vercel frontend URL (e.g. `https://your-app.vercel.app`). *Update this once the Vercel deployment is completed.* |
...
* **Build Command:** `npm run build -w shared && npm run build -w backend`

*How it works:* 
The build command runs the workspace builds in order. The backend build automatically executes `npx prisma generate` under the hood before compiling TypeScript code, ensuring the Prisma Client type bindings are generated and compiled cleanly.
...
4. Add the following **Environment Variable**:
   * **Key:** `BACKEND_API_URL`
   * **Value:** `https://ai-admission-backend.onrender.com` *(The Render Web Service URL copied in Step 6)*
...
* **Solution:** Check if `BACKEND_API_URL` is set correctly in Vercel's settings and contains the full Render Web Service URL (including `https://` but *no* trailing slash). Ensure your Render backend is running by navigating to `https://your-backend.onrender.com/api/health`.
